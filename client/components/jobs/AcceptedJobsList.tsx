...
export default function AcceptedJobsList() {
  ...
  // View state: "accepted" or "applied"
  const [viewMode, setViewMode] = useState<"accepted" | "applied">("accepted");
  const [applications, setApplications] = useState<JobApplication[]>([]);
  const [applicationsLoading, setApplicationsLoading] = useState(false);
  const [applicationFilter, setApplicationFilter] = useState<ApplicationStatus | "">("");
  ...
  }, [isHydrated, isAuthenticated, hasAccess, filter, viewMode, fetchJobs, fetchStats]);
  ...
  const filteredApplications = applicationFilter
          ...
          <div className="bg-white rounded-lg shadow mb-4">
            <div className="flex flex-wrap border-b border-gray-200">
              {[
                ...
                <button
                  key={tab.key}
                  onClick={() => setApplicationFilter(tab.key as ApplicationStatus | "")}
                  className={`px-4 py-3 text-sm font-medium transition-colors ${
            ...
            <div className="space-y-4">
              {filteredApplications.length === 0 ? (
                <div className="bg-white rounded-lg shadow p-8 text-center">
                  ...
                  <p className="text-gray-500">Chưa ứng tuyển công việc nào</p>
                </div>
              ) : (
