...
export default function HistoryTab({ jobs, isLoading }: HistoryTabProps) {
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${JOB_STATUS_CONFIG[job.status as keyof typeof JOB_STATUS_CONFIG]?.color}`}>
                    {JOB_STATUS_CONFIG[job.status as keyof typeof JOB_STATUS_CONFIG]?.label}
                  </span>
                </div>
